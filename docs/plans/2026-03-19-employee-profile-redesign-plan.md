# Employee Profile Page — Redesign Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the sparse 5-tab layout with a single rich scrollable page that gives managers/HR a full picture, and gives employees a permission-gated view of their own data.

**Architecture:** Server component (`page.tsx`) does all data fetching; layout sections are rendered inline with no sub-components (YAGNI). Permissions gate entire sections from the DOM — no client-side hiding. `profile-tabs.tsx` is deleted.

**Tech Stack:** Next.js 14 App Router (server components), Supabase, shadcn/ui (Card, Badge, Avatar, Button), lucide-react, date-fns

---

## Task 1: DB Migration — add `shared_with_employee` to `continuous_feedback`
...
Use the Supabase MCP tool (`mcp__supabase__apply_migration` or `mcp__e1e21cfb...apply_migration`) to run:
...
Project ref: `zhfvxfvmdlpdfgxrwtdn`
...
WHERE table_name = 'continuous_feedback'
  AND column_name = 'shared_with_employee';
```
...
```bash
git add -A
git commit -m "feat: add shared_with_employee column to continuous_feedback"
```

---

## Task 2: Rewrite `page.tsx` — scrollable layout, all sections

**Files:**
- Modify: `src/app/dashboard/team/[id]/page.tsx` (full rewrite)

Replace the entire file with the following. Read the current file first to confirm you're replacing the right content.
...
export default async function EmployeeProfilePage({
  ...
  const latestReview = reviewAssignments[0] ?? null;
...
Expected: no errors in `team/[id]/page.tsx`. Fix any type errors before proceeding.

### Step 3: Commit

```bash
git add src/app/dashboard/team/[id]/page.tsx
git commit -m "feat: redesign employee profile — scrollable single-page layout"
```

---

## Task 3: Delete `profile-tabs.tsx`

**Files:**
- Delete: `src/app/dashboard/team/[id]/profile-tabs.tsx`

### Step 1: Confirm it's no longer imported

```bash
grep -r "profile-tabs" "/Users/filipnowakowski/Test - Slack/feedback-app/src"
```

Expected: no results (the import was removed in Task 2).

### Step 2: Delete the file

```bash
rm "/Users/filipnowakowski/Test - Slack/feedback-app/src/app/dashboard/team/[id]/profile-tabs.tsx"
```
...
```bash
git add -A
git commit -m "chore: delete orphaned profile-tabs.tsx"
```
...
Verify:
- [ ] No tabs visible
- [ ] Header shows large avatar, name, title, chips, email, reports-to link
- [ ] 4 KPI tiles visible
- [ ] At-a-glance row: Latest Review card + Active Goals card (or tasteful empty states)
- [ ] Competency section present (manager) or absent (non-manager)
- [ ] Feedback section present (manager/own) or absent (other)
- [ ] Performance Reviews list
- [ ] Goals list with progress bars
- [ ] Direct Reports section only if person has reports
