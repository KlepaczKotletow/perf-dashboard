# Functions & Skills Framework — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the split job-families + competencies pages with a single unified "Functions" admin page — left sidebar lists functions, right panel shows levels + skills scorecard with inline expected-score editing.

**Architecture:** New client component `FunctionsClient` at `/dashboard/admin/functions` replaces both `/dashboard/admin/job-families` and `/dashboard/competencies`. One DB migration adds `job_family_id` to `competencies`. Old routes redirect. Nav is updated.

**Tech Stack:** Next.js 14 App Router, Supabase SSR, shadcn/ui, Tailwind CSS, lucide-react. Supabase project ref: `zhfvxfvmdlpdfgxrwtdn` (use MCP tool `mcp__supabase__execute_sql` or `mcp__supabase__apply_migration`).

---

## Reference: Key Patterns in This Codebase

- Server pages: import `createServerSupabaseClient, getUserWorkspace` from `@/lib/supabase-server`; role helpers `isAdminOrAbove`, `isHROrAbove`, `isManagerOrAbove` from `@/lib/roles`
- Client components: `"use client"`, `createBrowserClient` from `@supabase/ssr` — wrap in `useMemo(() => createBrowserClient(...), [])` to prevent re-creation
- Workspace guard: always read `user.user_metadata?.workspace_id` and return early if falsy before any insert
- Mutation pattern: `try/catch`, `setError(msg)` on failure, `router.refresh()` on success
- Nav lives in `src/app/dashboard/layout.tsx` — array of `{ href, label, icon, requiresManager, requiresAdmin }`

---

## Task 1: DB Migration — Add `job_family_id` to `competencies`

**Files:**
- No file — apply via Supabase MCP tool

**Step 1: Apply the migration**

Use the Supabase MCP tool with project ref `zhfvxfvmdlpdfgxrwtdn`:

```sql
ALTER TABLE competencies
ADD COLUMN IF NOT EXISTS job_family_id uuid REFERENCES job_families(id) ON DELETE CASCADE;
```
...
WHERE table_name = 'competencies' AND column_name = 'job_family_id';
```

Expected: one row with `column_name = job_family_id`, `is_nullable = YES`.

**Step 3: Commit**

```bash
git add -A
git commit -m "feat: add job_family_id to competencies table"
```
...
**Files:**
- Create: `src/app/dashboard/admin/functions/page.tsx`
...
**Step 2: Verify the file compiles (no errors yet since FunctionsClient doesn't exist)**

No test to run yet — file will be validated when FunctionsClient is created in Task 3.

**Step 3: Commit**

```bash
git add src/app/dashboard/admin/functions/page.tsx
git commit -m "feat: add functions page server component"
```

---

## Task 3: Create the FunctionsClient Component

**Files:**
- Create: `src/app/dashboard/admin/functions/functions-client.tsx`

This is the main component. Build it in layers:

### Step 1: Scaffold the file with types, imports, and empty component
...
Expected: no errors related to `functions-client.tsx`. Fix any type errors before proceeding.

### Step 3: Commit

```bash
git add src/app/dashboard/admin/functions/
git commit -m "feat: add FunctionsClient split-panel component"
```
...
**Step 1: Find the nav items array**

In `layout.tsx`, find the nav items array. Look for the entry with `href: "/dashboard/admin/job-families"` and the one with `href: "/dashboard/competencies"`.

**Step 2: Update the entries**
...
(Note: if `Target` icon is no longer used after removing this item, also remove it from the import.)

**Step 3: Verify the sidebar renders correctly by running the dev server briefly**
...
git commit -m "feat: update nav — rename Job Families to Functions, remove Competencies item"
```

---

## Task 5: Redirect Old Routes

**Files:**
- Modify: `src/app/dashboard/admin/job-families/page.tsx`
- Modify: `src/app/dashboard/competencies/page.tsx`

**Step 1: Replace job-families page with redirect**

Replace the entire contents of `src/app/dashboard/admin/job-families/page.tsx` with:

```tsx
import { redirect } from "next/navigation";

export default function JobFamiliesPage() {
  redirect("/dashboard/admin/functions");
}
```

**Step 2: Replace competencies page with redirect**

Replace the entire contents of `src/app/dashboard/competencies/page.tsx` with:
...
```bash
git add src/app/dashboard/admin/job-families/page.tsx src/app/dashboard/competencies/page.tsx
git commit -m "feat: redirect old job-families and competencies routes to /functions"
```
...
Verify:
- [ ] `/dashboard/admin/functions` loads with sidebar + function list
- [ ] Clicking a function shows its levels + scorecard
- [ ] Adding a function works (name input → Enter → sidebar updates)
- [ ] Adding a level works (pill appears)
- [ ] Clicking a score cell opens the 1–5 picker
- [ ] Setting a score persists after page reload
- [ ] `/dashboard/admin/job-families` redirects to `/dashboard/admin/functions`
- [ ] `/dashboard/competencies` redirects to `/dashboard/admin/functions`
- [ ] Sidebar nav shows "Functions" (not "Job Families"), no "Competencies" item

**Step 4: Final commit (if any fixes needed)**

```bash
git add -A
git commit -m "fix: post-deploy corrections for functions page"
```
...
Delete the now-unused client components from the old directories:

```bash
# Only do this after confirming redirects work in production
rm -rf "/Users/filipnowakowski/Test - Slack/feedback-app/src/app/dashboard/competencies"
rm -rf "/Users/filipnowakowski/Test - Slack/feedback-app/src/app/dashboard/admin/job-families"
git add -A
git commit -m "chore: remove deprecated competencies and job-families directories"
```

> ⚠️ Only run cleanup after the redirects are confirmed working in production, since removing the files removes the redirect handlers too. Keep the redirect-only page.tsx files, delete only the client components, actions, and matrix subdirectory.
