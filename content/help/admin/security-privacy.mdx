...
description: "How Perf protects your data — tenant isolation, encryption, compliance, and data handling"
...
Perf takes the security and privacy of your performance data seriously. This article explains how your data is protected, who can see what, and what commitments we make regarding your information.

## Who Can See What

Perf enforces strict role-based visibility at the API level — not just the UI. Even if someone manipulates a URL or API call, they cannot access data outside their permissions.
...
Each Slack workspace maps to a single Perf tenant. Your data is strictly isolated from all other organizations using multiple layers of protection:
...
When you sign in via Slack OAuth, Perf receives an access token scoped to only the permissions it needs:
...
| `app_mentions:read` | Detect when the Perf bot is mentioned |

<Callout type="warning">
Perf never reads your Slack channels, group messages, files, or general message history. The integration is limited to authentication, directory sync, and Nami bot direct messages.
</Callout>

## What Data We Store

Perf stores **workplace performance data** — review ratings, feedback, goals, competencies, and survey responses. We do **not** store or require:
