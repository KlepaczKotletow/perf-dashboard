-- Block cycle launches when any 'standard' assignment has a NULL manager_id.
-- Otherwise the manager_review phase opens but nobody can submit, which
-- leaves the cycle stuck — the cycle_completion guard then never fires.
-- Error code 23514 (check_violation) is caught client-side for the nicer
-- "assign managers in Team Settings first" message.
...
BEGIN
  ...
  -- Validate: every 'standard' assignment must have a manager_id. Pick up to
  -- five names for the error message so the admin can act on it directly.
  SELECT
    COUNT(*),
    string_agg(u.slack_name, ', ' ORDER BY u.slack_name)
  INTO v_missing_mgr, v_sample_names
  FROM jsonb_array_elements(p_assignments) x
  JOIN users u ON u.id = (x->>'employee_id')::uuid
  WHERE COALESCE(x->>'assignment_type','standard') = 'standard'
    AND (NULLIF(x->>'manager_id','') IS NULL OR (x->>'manager_id') IS NULL);

  IF v_missing_mgr > 0 THEN
    RAISE EXCEPTION
      'Cannot launch: % employee(s) have no manager assigned (%). Set their managers in Team Settings first.',
      v_missing_mgr, COALESCE(v_sample_names, '')
      USING ERRCODE = '23514';
  END IF;
